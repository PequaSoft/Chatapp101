    ...
    <template #header>
      <div class="flex items-center justify-between text-primary">
        <div class="flex items-center gap-x-2">
          ...
          />
          <div class="text-primary font-semibold text-center text-xl">Nuxtt Discord</div>
        </div>
        <div
          @click="() => navigateTo('/')"
...
onMounted(() => {
  ...
  //alert('Mounted: ' + JSON.stringify( socket))

  if (!username || !room) {
    //alert('Mounted Redirect')
    navigateTo('/');
  }
  socket.value = io({
    path: '/api/socket.io'
...
socket.value.on('roomUsers', (response: { room: string, users: User[] }) => {
  //alert('Mounted: ' + JSON.stringify( response))
  currentRoom.value = response.room
  users.value = response.users
})
//alert('Mounted x:' + JSON.stringify(users.value))
